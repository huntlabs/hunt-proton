...
module hunt.proton.codec.SymbolMapType;
...
interface MapEncoding : PrimitiveTypeEncoding!(Map!(Symbol,Object))
{
    void setValue(Map!(Symbol,Object) value, int length);
}

class SymbolMapType : AbstractPrimitiveType!(Map!(Symbol,Object))
{
    ...
    this(EncoderImpl encoder, DecoderImpl decoder)
    {
        ...
        encoder.register(typeid(LinkedHashMap!(Symbol,Object)), this);
        decoder.register(this);
    }

    public TypeInfo getTypeClass()
    {
        return typeid(LinkedHashMap!(Symbol,Object));
    }
    ...
    {
        int calculatedSize = calculateSize(cast(Map!(Symbol,Object))val);
        logInfof("##########  %d" , calculatedSize);
        MapEncoding encoding = ((cast(Map!(Symbol,Object))val).size() > 127 || calculatedSize >= 254)
        ? _mapEncoding
        : _shortMapEncoding;

        encoding.setValue(cast(Map!(Symbol,Object))val, calculatedSize);
        return encoding;
    }

    private int calculateSize(Map!(Symbol,Object) map)
    {
        ...
        try
        {
            foreach (MapEntry!(Symbol, Object) element ; map)
            {
    ...
    //Collection!(TypeEncoding!(Map!(Symbol,Object)))
    public Collection!(TypeEncoding!(Map!(Symbol,Object))) getAllEncodings()
    {
        ArrayList!(TypeEncoding!(Map!(Symbol,Object))) lst = new ArrayList!(TypeEncoding!(Map!(Symbol,Object)))();
    ...
    : LargeFloatingSizePrimitiveTypeEncoding!(Map!(Symbol,Object))
    , MapEncoding
    {
        ...
        override
        protected void writeEncodedValue(Map!(Symbol,Object) map)
        {
            ...
            try {
                foreach (MapEntry!(Symbol, Object) element ; map)
                {
                    ...
                    if (IfixedKeyType is null)
                    {
                        SymbolType tmp = cast(SymbolType)(_encoder.getType( element.getKey()));
                        if (tmp is null)
                        {
                            logError( "getType Error");
                        }
        ...
        override
        protected int getEncodedValueSize(Map!(Symbol,Object) val)
        {
        ...
        override
        public SymbolMapType getType()
        {
            return this.outer;
        }

        override
        public bool encodesSuperset(TypeEncoding!(Map!(Symbol,Object)) encoding)
        {
        ...
        public Object readValue()
        {
            ...
            for(int i = 0; i < count / 2; i++)
            {
                ...
                Symbol key = cast(Symbol)keyConstructor.readValue();
        ...
        public void setValue(Map!(Symbol,Object) value, int length)
        {
            _value = value;
            _length = length;
        }
    ...
    : SmallFloatingSizePrimitiveTypeEncoding!(Map!(Symbol,Object))
    , MapEncoding
    {
        private Map!(Symbol,Object) _value;
        private int _length;
        ...
        override
        public bool encodesSuperset(TypeEncoding!(Map!(Symbol,Object)) encoder)
        {
            return encoder == this;
        }
